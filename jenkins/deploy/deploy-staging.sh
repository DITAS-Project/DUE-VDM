...
sudo docker stop --time 20 due-vdm || true
sudo docker rm --force due-vdm || true
sudo docker pull ditas/due-vdm:staging

# Get the host IP
HOST_IP="$(ip route get 8.8.8.8 | awk '{print $NF; exit}')"

# Run the docker mapping the ports and passing the host IP via the environmental variable "DOCKER_HOST_IP"
sudo docker run -p 30022:5000 -e DOCKER_HOST_IP=$HOST_IP --restart unless-stopped -d --name due-vdm ditas/due-vdm:staging
ENDSSH
